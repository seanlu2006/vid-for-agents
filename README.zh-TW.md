...
1. **這是抽樣，不是「看影片」。** agent 拿到的是 N 張靜態影格。短影片抽 12 張大致等於看完，但**快速閃過的字卡、連續動作的細節、逐格變化的動畫**一定會漏。這類影片請把 `-n` 調到 30 以上，或者接受逐字稿才是主要資訊來源。OCR 只讀得到有抽到的那幾張；剛好落在兩張之間的字卡，一樣會漏。
...
測試用真的 `ffmpeg` 產生測試影片，`whisper-cli` 和 `curl` 則換成假的替身，所以幾秒就跑完，也不用下載模型。涵蓋的都是曾經壞過的情境：純音訊檔、音軌比畫面長、沒給 checksum 的自訂模型、checksum 錯誤、下載失敗、重複句偵測、VAD 與各種退路，還有非 UTF-8 的 locale。OCR 測試是例外，不用替身：它會畫出字卡、做成影片，再用真的 Vision 去辨識，所以需要 macOS 和 `swiftc`。每次 push，CI 都會跑 ShellCheck 和這支測試，並設 `VID_REQUIRE_OCR=1`，OCR 測試不能被默默跳過。
