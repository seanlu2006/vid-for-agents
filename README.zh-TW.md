...
| 下載並驗證模型 | 語音模型約 547 MB、Silero VAD 模型約 0.9 MB，存到 `~/.cache/whisper-models/`，一次性；會檢查固定的上游版本與 SHA-256 |
| 建立 symlink | `~/bin/vid` → 你 clone 下來的 `vid` |
| 檢查 PATH 與 `swiftc` | 若 `~/bin` 不在 PATH，**只印出**該加的那行給你自己執行；沒有 `swiftc` 就提醒你 OCR 會跳過、要怎麼裝 |
...
| `swiftc`（Xcode Command Line Tools） | 用 macOS Vision 辨識畫面文字 | 跳過 OCR，其他照跑 |
...
在 macOS 上，`vid` 還會辨識每張影格上的文字：燒在畫面上的字幕、字卡、投影片上的條列。結果附在 README 每張影格底下，agent 不用開任何一張圖，就知道畫面上寫了什麼。燒錄字幕常常好幾張都一樣，所以跟上一張完全相同時只寫「（同上一張）」，不重複貼。完全沒有文字的影格則寫「（無）」，比指回上一張還短，也不用往回找。原始結果另外存在 `ocr.json`。
...
| `--no-ocr` | 不辨識畫面文字 | macOS 上會辨識 |
| `--no-vad` | 整條音軌連靜音一起丟給 whisper | 先濾掉靜音 |
| `--uniform` | 影格一律均勻抽樣，不看場景變化 | 會混入場景變化 |
...
輸入 ─┬─ URL ──► yt-dlp ──► 影片檔 + info.json（標題/作者/貼文原文/讚數）
   ...
   ffmpeg                      ffmpeg
      │                           │
 場景變化 + 均勻影格          16kHz 單聲道 WAV
  (JPEG, 寬 ≤768px)               │
      │                    Silero VAD（濾掉靜音）
  macOS Vision OCR                │
  （畫面文字）             whisper.cpp（本機，Apple Silicon 走 Metal）
      │                           │
      │                     transcript.txt / .srt
      │                           │
      ...
      └─────────────┬─────────────┘
                    │
        產生 README.md（metadata + 逐字稿全文 + 影格與畫面文字）
```

### 各階段的設計取捨

**抽樣**：四分之一的張數給場景變化最劇烈的時間點，其餘平均分散在整支影片上。場景分數用 ffmpeg 的 `scdet` 濾鏡算，先降到 6fps、320px 寬再算，5 分鐘的影片花 0.8 秒；抽的時間點是轉場後 0.35 秒，避免抽到轉場中間。均勻的那幾張取在 `t = 畫面長度 × (i − 0.5) / N`，也就是每個等分區間的中點而非端點，避開頭尾常見的黑畫面。這裡的長度是畫面串流的長度，不是整個檔案的：Reels 常把 3 秒畫面配上 10 秒音樂，照檔案長度抽，大部分影格會落在畫面已經結束的地方。

為什麼要混合？因為兩種各有輸贏。我拿一張只出現 0.6 秒的快閃字卡測：均勻抽樣完全漏掉，還把四張裡的兩張花在同一個畫面；場景偵測則抓到了。但換成 5 分鐘的投影片錄影就反過來，因為畫面變化來自逐步畫上去的標註，場景分數會擠在那附近。配額調到四分之一時，快閃字卡照樣抓得到，投影片錄影也維持原本 11 個不重複畫面。`--uniform` 可以把場景那一半關掉。

影格縮到寬度 768px（原本就更窄就不放大），JPEG 品質 `-q:v 4`。這個尺寸剛好在多數 vision 模型的處理解析度附近，再大只是浪費 token。
...
1. **這是抽樣，不是「看影片」。** agent 拿到的是 N 張靜態影格。短影片抽 12 張大致等於看完，但**連續動作的細節、逐格變化的動畫**一定會漏。場景偵測抓得到大部分字卡，我測過只出現 0.6 秒的也抓得到，但沒有明顯切換就冒出來的字卡，還是可能落在兩張之間。這類影片請把 `-n` 調到 30 以上，或者接受逐字稿才是主要資訊來源。
...
4. **中文逐字稿會有錯字。** whisper 對中文的專有名詞、人名、英文夾雜的段落辨識率明顯低於英文，opencc 也只能修字形不能修辨識錯誤。當草稿看，別當引用來源。另外沒裝 `opencc` 的話輸出會停在簡體。
   ...
   `opencc` 的台灣詞庫也偏軟體領域：它把「类型」轉成寫程式才講的「型別」，在一支交易教學片裡錯了 6 次。`vid` 只把這個改回「類型」，其餘轉換（軟體、記憶體、影片、資訊、網路）都保留。要加自己的對應：`VID_OPENCC_FIXES="型別=類型,專案=項目"`。

5. **長影片可能讓 whisper 鬼打牆。** 超過 15 分鐘左右的錄音，whisper 偶爾會卡住，同一句重複好幾百行。我跑過 8 支長的教學影片，有 2 支中招。`vid` 預設加 `-mc 0`，每一段辨識都不沿用前文，也會先跑 Silero VAD，靜音和純音樂的段落根本不會進 whisper。那 2 支（21 和 23 分鐘）我用各種設定重跑過，兩招各自都能擋住鬼打牆。兩招一起用，在 M5 上轉錄時間還少了約四分之一（32 → 24 秒、34 → 25 秒），字數差不到 1%，時間軸也對得上原片。另外還有一道保險：同一句連續出現 15 次以上，`逐字稿` 標題就會加上警告。看到警告，那一段別信。

6. **OCR 偶爾錯字，也會把左右兩欄混在一起。** 會有單字的錯，例如某次測試把「的」讀成「約」。文字是由上往下排的，所以左邊是文字、右邊是有標註的圖表時，兩邊的字會交錯。OCR 只在 macOS 上跑，其他平台直接跳過。

7. **只在 macOS 測試過。** 核心邏輯是標準 POSIX 工具，Linux 理論上把 `brew` 換成 `apt` / `pacman` 就能跑，但沒驗證過，`install.sh` 也會直接拒絕在非 macOS 執行。Windows 沒測過。
...
測試用真的 `ffmpeg` 產生測試影片，`whisper-cli` 和 `curl` 則換成假的替身，所以幾秒就跑完，也不用下載模型。共 23 項，涵蓋的都是曾經壞過的情境：純音訊檔、音軌比畫面長、沒給 checksum 的自訂模型、checksum 錯誤、下載失敗、重複句偵測、VAD 與各種退路、opencc 之後的用詞修正，還有非 UTF-8 的 locale。OCR 測試是例外，不用替身：它會畫出字卡、做成影片，再用真的 Vision 去辨識，所以需要 macOS 和 `swiftc`。每次 push，CI 都會跑 ShellCheck 和這支測試，並設 `VID_REQUIRE_OCR=1`，OCR 測試不能被默默跳過。

---

## 同類專案

[`claude-real-video`](https://github.com/HUANGCHIHHUNGLeo/claude-real-video)（2.1k 星、MIT）在回答同一個問題，而且比這個專案早三個月開始。它做得更多：場景偵測加滑動視窗去重、講者分離、MCP server 與 Claude Code plugin、網頁檢視器。想要功能完整的工具，用那個。

這裡的差別很窄：`vid` 是單一一個 bash 檔案，不用裝 Python 套件；繁體中文這條路是我自己每天在用的——whisper 的簡體輸出轉成台灣正體、把偏軟體的用詞改回來，OCR 也固定用繁中而不是讓系統自己猜。就這樣而已。
