...
whisper 執行失敗或影片根本沒有音軌時，`## 逐字稿` 這個標題後面會多一段說明原因的括號；失敗的那次還會把 `audio.wav`（16kHz 單聲道）留在資料夾裡，讓你手動重試。
...
- 輸出目錄名稱是 `<標題 slug>-<YYYYMMDD-HHMMSS>`，帶時間戳所以不會互相覆蓋。slug 保留英數字與漢字（U+4E00–U+9FFF），其餘字元一律收斂成單一 `-`，並截斷在 50 個字元。所以日文假名、韓文、西里爾字母、帶重音的拉丁字母都會被吃掉：`アニメの作り方` 會變成 `作-方`，整個標題都是這類字元的話會退回預設名 `video`。
